    ...
    <div>
      ...
      <div style="margin-top: 10px">
        <el-button
          ...
          <i v-if="bookInfo.isFavourite" class="fa fa-star" style="color: #ff9900"></i>
      ...
      <div>
        <el-button
          ...
          style="width: 100%"
          @click="getPrevPic"
          :disabled="prevPics.length === previewInfo.totalPages">{{ prevPics.length === previewInfo.totalPages ?
          '已加载全部预览图' : '加载更多预览图'}}
        </el-button>
      </div>
    </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      toggleFavourite () {
        this.$store.dispatchPromise('UpdateBookInfo', { isFavourite: !this.bookInfo.isFavourite }).then(() => {
